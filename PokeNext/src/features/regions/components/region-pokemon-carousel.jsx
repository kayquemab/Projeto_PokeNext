...
import { useRegionPokedexPokemon } from "../hooks/use-regions-api";
...
export default function RegionPokemonCarousel({ region, spaceBetween = 1 }) {
    ...
    const cards = useRegionPokedexPokemon(regionToPokedex[region]);
        ...
        <div className="w-full pb-10 pt-8 lg:pt-0 select-none relative overflow-visible">
            <div className="relative w-full mx-auto">
                ...
                <h3
                    ...
                    <Image src="/pokeball.png" alt="Pokébola" width={22} height={22} className="w-[22px] h-[22px] opacity-30 brightness-0 invert object-contain" />
                    Pokémons da Região
                </h3>
                                ...
                                <SwiperSlide key={p.id} style={{ width: "330px" }}>
                                    ...
                                    >
                                        ...
                                        <div className={`relative bg-neutral-600 overflow-hidden transition-all duration-300 ${isActive ? "h-[260px]" : "h-[230px]"}`}>
                                            ...
                                            <Image src={p.image} alt={p.displayName} fill sizes="100%" className={`absolute inset-0 object-contain opacity-100 brightness-110 z-10 transition-all duration-300 ${isActive ? "scale-105" : "scale-90"}`} />
                                        </div>
                                            ...
                                            {isActive && (
                                                <div className="relative translate-y-4">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {p.types.map((type) => (
                                                            <span key={type.code} className="text-white text-sm font-medium px-2 py-1 rounded-md capitalize" style={{ backgroundColor: typeColors[type.code] }}>
                                                                {type.label}
                                                            </span>
                                                        ))}
                                                    </div>
                                                    {mainAbility && (
                                                        <div className="flex items-center gap-2 mt-1">
                                                            ...
                                                            <span className="text-zinc-100 text-sm capitalize truncate">{mainAbility.label}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            )}
