...
import { useMovesByPokemonCarousel } from "../hooks/use-moves-by-pokemon-carousel";
...
export default function MovimentosPokemonCarrousel({
    ...
    const { cards, detailsByMove, goToMove, realIndex, setRealIndex } =
        useMovesByPokemonCarousel({ count, pokemonId });
                ...
                <div className="relative w-full h-[350px]">
                    ...
                    >
                        {cards.map((m, index) => {
                            ...
                            const d = detailsByMove[m.slug];
                            const types = Array.isArray(d?.types) ? d.types.slice(0, 2) : [];

                            return (
                                <SwiperSlide key={m.slug} style={{ width: "330px" }}>
                                    <motion.div
                      ...
                      cursor-pointer
                    `}
                                        onClick={() => goToMove(m.slug)}
                                        role="button"
                                        tabIndex={0}
                                        onKeyDown={(e) => {
                                            if (e.key === "Enter") goToMove(m.slug);
                                        }}
                                    >
                                                ...
                                                {d ? (
                                                    <div className="mt-2 text-sm text-white/90 space-y-1">
                                                        ...
                                                        <div className="flex items-center justify-center gap-4 flex-wrap">
                                                            <span>
                                                                Poder: <span className="font-semibold">{d.power ?? "-"}</span>
                                                            </span>
                                                            <span>
                                                                Precisão: <span className="font-semibold">{d.accuracy ?? "-"}</span>
                                                            </span>
                                                            <span>
                        ...
                        `}
                                            >
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    />
                                                    <span className="capitalize">{m.displayName}</span>
                                                </div>
                                            </h4>
                                                        ...
                                                        {types.length ? (
                                                            types.map((tp) => (
                                                                <span
                                                                    ...
                                                                    style={{ backgroundColor: typeColors[tp.code] || typeColors.default }}
                                                                >
                                                                    {tp.label}
                                                                </span>
                                                            ))
