...
import { useStoredTeam } from "../hooks/use-stored-team";
...
function TypeBadge({ type, label }) {
    return (
        ...
        >
            {label || TYPE_LABELS[type] || "Tipo desconhecido"}
        </span>
    );
}
...
export default function TeamAnalysis({ team = [] }) {
    const savedTeam = useStoredTeam(STORAGE_KEY);
    const { analyzeTeam } = useTeamsApi();

    const analysisTeam = team.length > 0 ? team : savedTeam;

    const analysisData = useMemo(
        () =>
            analyzeTeam(analysisTeam, {
                statConfig: STAT_CONFIG,
                typeWeaknesses: TYPE_WEAKNESSES,
            }),
        [analysisTeam, analyzeTeam]
    );
